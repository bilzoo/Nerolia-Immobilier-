...
export function Tabs({ value, onValueChange, children }: TabsProps) {
  return <div data-value={value}>{React.Children.map(children, (c: any) => React.cloneElement(c, { __tabsValue: value, __onChange: onValueChange }))}</div>;
}
...
export function TabsTrigger({ value, children, __tabsValue, __onChange, ...props }: any) {
  ...
  return (
    <button
      className={`px-3 h-8 rounded-lg text-sm ${active ? "bg-white shadow border border-slate-200" : "opacity-70 hover:opacity-100"}`}
